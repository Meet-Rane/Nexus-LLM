        ...
        {!collapsed && (
          <div className="mb-2 flex items-center gap-3 rounded-xl px-2 py-2">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-semibold text-text">{user?.username}</p>
              <p className="truncate font-mono text-[9px] text-muted">{roleLabel(user?.roles)}</p>
            </div>
            <button type="button" title="Sign out" onClick={signOut} className="icon-button h-7 w-7"><LogOut size={13} /></button>
          </div>
        )}
        <button
